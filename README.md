...
*nop, no out of LAN control right now. Im using VPN to home network so if i need it so i didnt put this on focus, easy to implement something if there is interest to control prints from other side of the world. Really decent ways to do it will cost me abit, dont wanna run server at home. But i have some ideas still how to get control without some servers also*
